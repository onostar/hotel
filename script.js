...
function calculateDays(){
     ...
     num_days.innerHTML = "<label for='amount_due' style='color:red'>Checking in for</label><br><input type='text' style='color:var(--primaryColor);border:none;font-size:1rem' readonly value='"+totalDays+" day(s)'>";
     // alert(totalDays);
...
function checkIn(){
     ...
     if(check_in_category.length == 0 || check_in_category.replace(/^\s+|\s+$/g, "").length == 0){
          ...
          $.ajax({
               ...
               data : {posted_by:posted_by,check_in_room:check_in_room, last_name:last_name, first_name:first_name, emergency_contact:emergency_contact, gender:gender,contact_address:contact_address, contact_phone:contact_phone, check_in_date:check_in_date, check_out_date:check_out_date, amount_due:amount_due},
               success : function(response){
               $("#check_in").html(response);
               }
          })
     ...
     if(confirmPost){
          ...
          if(payment_type.length == 0 || payment_type.replace(/^\s+|\s+$/g, "").length == 0){
               ...
               $.ajax({
                    type : "POST",
                    url : "../controller/post_payments.php",
                    data : {sales_invoice:sales_invoice, payment_type:payment_type, bank:bank, multi_cash:multi_cash, multi_pos:multi_pos, multi_transfer:multi_transfer, discount:discount, store:store, total_amount:total_amount, deposits:deposits, check_in_id:check_in_id},
                    success : function(response){
                         $("#all_payments").html(response);
                    }
               })
